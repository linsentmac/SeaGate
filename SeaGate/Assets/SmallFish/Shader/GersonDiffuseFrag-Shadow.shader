...
Shader "Gerson/CG shader Reflect"{
	Properties{
		reflectivity("reflectivity1", float) = 0 //反射系数  影响反射强度
...
#include "UnityCG.cginc" //注意引入
	...
	{
		VertOutput o;
		o.oPosition = UnityObjectToClipPos(input.position);//UNITY_MATRIX_MVP变量, 就是对应图形中的模型视图投影矩阵(ModelViewProj),unity中规定 必须这么写
