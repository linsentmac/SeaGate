...
public class ClickGifHint : MonoBehaviour {
    ...
    void Start () {
        System.Drawing.Image image = System.Drawing.Image.FromFile(Application.streamingAssetsPath + "/click_hint.gif");
	...
	void Update () {
        if (tex2DList.Count > 0 && isFirst)
        {
            ...
            Debug.Log("tmac Ims length = " + Ims.Length);
            if (Ims.Length != 0)
            {
                for (int i = 0; i < Ims.Length; i++) {
                    Ims[i].GetComponent<SpriteRenderer>().material.mainTexture = tex2DList[index];
                    Debug.Log("tmac Ims length ========================= ");
                }
            }
        }
    }
    ...
    private List<Texture2D> MyGif(System.Drawing.Image image)
    {
        ...
        if (image != null)
        {
            ...
            Debug.Log("Tmac image length：" + image.FrameDimensionsList.Length);
